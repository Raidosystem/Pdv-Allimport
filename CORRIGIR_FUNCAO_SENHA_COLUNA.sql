-- ========================================
-- CORRIGIR FUNÇÃO cadastrar_funcionario_simples
-- ========================================
-- Problema: Estava usando 'senha_hash' mas a coluna é 'senha'
...
BEGIN
  ...
  BEGIN
    INSERT INTO login_funcionarios (
      ...
      crypt(p_senha, gen_salt('bf')),
      true,
      true  -- Primeiro acesso = true (usuário deve trocar senha)
    );
  EXCEPTION
    WHEN OTHERS THEN
      RAISE WARNING 'Erro ao criar login_funcionarios: %', SQLERRM;
      -- Não é crítico, pois pode ser criado manualmente depois
  END;
...
-- Comentário
COMMENT ON FUNCTION cadastrar_funcionario_simples IS 'Cria funcionário com conta Supabase Auth + login local. Corrigido para usar coluna "senha" na tabela login_funcionarios.';

SELECT '✅ Função cadastrar_funcionario_simples CORRIGIDA - coluna senha' as status;
