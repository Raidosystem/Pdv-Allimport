...
console.log('🔄 Gerando SQL com UPSERT (atualizar ou inserir)...\n');
...
// Empresa ID (mesmo do backup original)
...
// Cabeçalho
sql += `-- =============================================\n`;
sql += `-- ATUALIZAÇÃO/IMPORTAÇÃO DE DADOS (UPSERT)\n`;
sql += `-- Gerado automaticamente em ${new Date().toLocaleString('pt-BR')}\n`;
sql += `-- Usa ON CONFLICT para atualizar dados existentes\n`;
sql += `-- =============================================\n\n`;

sql += `-- Desabilitar triggers temporariamente (opcional)\n`;
...
// Salvar arquivo
const outputPath = path.join(__dirname, 'atualizar-dados-upsert.sql');
...
console.log(`   - Produtos: ${produtos.length}`);
console.log(`   - Ordens de Serviço: ${ordens.length}\n`);
console.log(`🔄 Este arquivo usa UPSERT (ON CONFLICT DO UPDATE)`);
console.log(`   → Atualiza registros existentes`);
console.log(`   → Insere apenas os novos\n`);
console.log(`📋 Para executar:`);
console.log(`   1. Abra: https://supabase.com/dashboard/project/kmcaaqetxtwkdcczdomw`);
console.log(`   2. Vá em SQL Editor`);
console.log(`   3. Cole o conteúdo de atualizar-dados-upsert.sql`);
