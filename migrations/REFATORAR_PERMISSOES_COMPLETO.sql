-- =====================================================
-- ?? REFATORAÇÃO COMPLETA DO SISTEMA DE PERMISSÕES
-- =====================================================
-- Este script reorganiza TOTALMENTE as permissões do sistema
-- para que funcionem IMEDIATAMENTE ao editar no painel admin
-- =====================================================

BEGIN;

-- =====================================================
-- 1?? LIMPAR TUDO (Fresh Start)
-- =====================================================
DELETE FROM funcao_permissoes;
DELETE FROM permissoes;

-- =====================================================
-- 2?? CRIAR ESTRUTURA DE PERMISSÕES MODERNA
-- =====================================================
-- Agora com SEÇÕES e SUBSEÇÕES para facilitar gerenciamento
...
-- =====================================================
-- 4?? APLICAR PERMISSÕES AO ADMINISTRADOR
-- =====================================================
DO $$
DECLARE
...
BEGIN
  ...
  IF v_funcao_admin_id IS NULL THEN
    RAISE EXCEPTION '? Função Administrador não encontrada';
  END IF;
  ...
  SELECT 
    v_funcao_admin_id,
    p.id,
    NULL  -- Permissões globais
  FROM permissoes p
  ON CONFLICT DO NOTHING;
  
  RAISE NOTICE '? Permissões aplicadas ao Administrador';
END $$;

-- =====================================================
-- 5?? APLICAR PERMISSÕES AO VENDEDOR
-- =====================================================
DO $$
DECLARE
...
BEGIN
  ...
  IF v_funcao_vendedor_id IS NULL THEN
    RAISE NOTICE '?? Função Vendedor não encontrada (OK se não foi criada)';
    RETURN;
  END IF;
  
  -- Vendedor: vendas, clientes e produtos (apenas visualizar + criar)
  INSERT INTO funcao_permissoes (funcao_id, permissao_id, empresa_id)
  SELECT 
    v_funcao_vendedor_id,
    p.id,
    NULL
  FROM permissoes p
  WHERE 
    -- Vendas: read, create, update (SEM delete, cancel, refund)
    ...
    -- Produtos: read (SEM create, update, delete, manage_stock)
    OR (p.recurso = 'produtos' AND p.acao IN ('read'))
  ON CONFLICT DO NOTHING;
  
  RAISE NOTICE '? Permissões aplicadas ao Vendedor';
END $$;

-- =====================================================
-- 6?? CRIAR VIEW PARA HIERARQUIA DE PERMISSÕES
-- =====================================================
...
-- 7?? VERIFICAÇÃO FINAL
-- =====================================================
-- Ver todas as permissões organizadas
SELECT 
  '?? PERMISSÕES ORGANIZADAS' as info,
  categoria as "Seção",
  COUNT(*) as "Total",
...
SELECT * FROM v_permissoes_hierarquia LIMIT 20;

-- Contar permissões por função
SELECT 
  '?? PERMISSÕES POR FUNÇÃO' as info,
  f.nome as "Função",
  COUNT(fp.permissao_id) as "Total Permissões"
FROM funcoes f
...
-- - Vendas: 8 permissões (4 principais + 4 subseções)
-- - Produtos: 9 permissões (4 principais + 5 subseções)
-- - Clientes: 8 permissões (4 principais + 4 subseções)
-- - Caixa: 7 permissões (2 principais + 5 subseções)
-- - Ordens: 6 permissões (4 principais + 2 subseções)
-- - Relatórios: 7 permissões (1 principal + 6 subseções)
-- - Configurações: 11 permissões (2 principais + 9 subseções)
-- - Administração: 19 permissões (2 principais + 17 subseções)
-- 
-- Função Administrador: ~80 permissões (todas)
-- Função Vendedor: ~10 permissões (apenas operacionais)
-- =====================================================
