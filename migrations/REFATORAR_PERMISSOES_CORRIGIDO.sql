-- =====================================================
-- ?? REFATORAÇÃO PERMISSÕES - ORDEM CORRETA
-- =====================================================
-- CORRIGIDO: Cria colunas ANTES de usar
-- =====================================================

BEGIN;

-- =====================================================
-- 1?? ADICIONAR COLUNAS PRIMEIRO (ORDEM CORRETA!)
-- =====================================================
DO $$
BEGIN
...
-- =====================================================
-- 3?? CRIAR ESTRUTURA DE PERMISSÕES MODERNA
-- =====================================================
...
RAISE NOTICE '? 78 permissões criadas com sucesso';
...
BEGIN
  ...
  WHERE funcao_id = v_funcao_admin_id;
  
  RAISE NOTICE '? Administrador: % permissões aplicadas', v_count;
END $$;
...
BEGIN
  ...
  IF v_funcao_vendedor_id IS NULL THEN
    RAISE NOTICE '?? Função Vendedor não encontrada (pulando)';
    RETURN;
  END IF;
  
  -- Vendedor: vendas, clientes e produtos (apenas básico)
  INSERT INTO funcao_permissoes (funcao_id, permissao_id, empresa_id)
  SELECT 
  ...
  FROM permissoes p
  WHERE 
    -- Vendas: read, create, update, print (SEM delete, cancel, refund)
    (p.recurso = 'vendas' AND p.acao IN ('read', 'create', 'update', 'print'))
    -- Clientes: read, create, update, view_history (SEM delete)
    OR (p.recurso = 'clientes' AND p.acao IN ('read', 'create', 'update', 'view_history'))
    -- Produtos: APENAS read (SEM create, update, delete)
  ...
  RAISE NOTICE '? Vendedor: % permissões aplicadas', v_count;
END $$;

-- =====================================================
-- 6?? CRIAR VIEW PARA HIERARQUIA
-- =====================================================
...
RAISE NOTICE '? View v_permissoes_hierarquia criada';
...
SELECT 
  ...
  f.nome as "Função",
  COUNT(fp.permissao_id) as "Total"
FROM funcoes f
LEFT JOIN funcao_permissoes fp ON fp.funcao_id = f.id
GROUP BY f.nome
ORDER BY f.nome;

-- Ver amostra da hierarquia
SELECT * FROM v_permissoes_hierarquia LIMIT 10;

COMMIT;

-- =====================================================
-- ? PRONTO! SISTEMA DE PERMISSÕES REFATORADO
-- =====================================================
-- 
-- Resultado Esperado:
-- - 78 permissões criadas
-- - Função Administrador: ~78 permissões
-- - Função Vendedor: ~9 permissões
-- - View de hierarquia criada
-- 
-- Próximos passos:
-- 1. Testar interface admin de edição de permissões
-- 2. Fazer logout/login com Jennifer
-- 3. Verificar que mudanças aplicam imediatamente
-- =====================================================
