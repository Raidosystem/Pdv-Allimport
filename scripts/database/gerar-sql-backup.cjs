...
const values = backup.data.map((produto, index) => {
  ...
  // Tratar código de barras especialmente - remover aspas inválidas primeiro
  let codigoBarras = produto.barcode || '';
  // Remover aspas simples do início se houver
  if (codigoBarras.startsWith("'")) {
    codigoBarras = codigoBarras.substring(1);
  }
  // Depois escapar aspas normalmente
