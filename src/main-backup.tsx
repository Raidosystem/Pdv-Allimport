...
// Adicionar CSS para animações PWA
const addPWAStyles = () => {
...
const setupPWAInstall = () => {
  ...
  // Mostrar botão em produção ou localhost (para teste)
  ...
  if (isProduction || isLocalhost) {
    ...
    console.log('⚙️ PWA botão desabilitado')
  }
}
...
// Setup do sistema PWA (Funciona em produção e desenvolvimento)
const setupPWAInstall = () => {
  addPWAStyles()
  
  // Mostrar botão em produção ou se for em localhost (para teste)
  const isProduction = !import.meta.env.DEV
  const isLocalhost = location.hostname === 'localhost' || location.hostname === '127.0.0.1'
  
  // Mostrar botão se está em produção OU se é localhost para teste
  if (isProduction || isLocalhost) {
    console.log('� Botão PWA: Mostrando...')
    setTimeout(showInstallButton, 500)
  } else {
    console.log('⚙️ PWA botão desabilitado (desenvolvimento)')
  }
}
