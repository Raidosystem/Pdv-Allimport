...
-- Ver quais IDs do backup NÃO estão no Supabase
WITH backup_ids AS (
  SELECT unnest(ARRAY[
    ...
    -- Adicione TODOS os 141 IDs aqui
  ])::uuid AS id
)
SELECT 
  b.id as id_faltante,
  'Cliente do backup não encontrado no Supabase' as status
FROM backup_ids b
LEFT JOIN clientes c ON c.id = b.id AND c.empresa_id = 'f1726fcf-d23b-4cca-8079-39314ae56e00'
WHERE c.id IS NULL;

-- Ver clientes sem telefone
SELECT 
  id,
  nome,
  cpf_cnpj,
  'SEM TELEFONE' as problema
FROM clientes
WHERE empresa_id = 'f1726fcf-d23b-4cca-8079-39314ae56e00'
  AND (telefone IS NULL OR telefone = '');

-- Ver clientes sem CPF
SELECT 
  id,
  nome,
  telefone,
