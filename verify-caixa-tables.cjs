...
async function verificarTabelasCaixa() {
  ...
  try {
    ...
    if (caixaTest.status === 200) {
      ...
      mostrarInstrucoes();
      return;
    } else if (caixaTest.status === 401) {
      console.log('✅ Tabela "caixa" existe (erro de autenticação é normal)');
    } else {
    ...
    if (movTest.status === 200) {
      ...
      mostrarInstrucoes();
      return;
    } else if (movTest.status === 401) {
      console.log('✅ Tabela "movimentacoes_caixa" existe (erro de autenticação é normal)');
    } else {
